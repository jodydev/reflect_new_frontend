...
export default function CardNews() {
  ...
  useEffect(() => {
    fetch("https://api.allorigins.win/get?url=https://medium.com/feed/@jodyossino.dev")
      .then((res) => res.json())
      .then((data) => {
        ...
        const posts = items.slice(0, 3).map((item) => {
          ...
          if (contentEncoded) {
            ...
            const paragraphs = Array.from(pElements).map((p) => p.innerText).join('<br/>'); // Unisci i paragrafi con un <br> tra di loro
                ...
                <div className="absolute top-5 right-8">
                  <Squircle radius={90}>
                    <span className="bg-primary w-[5000px] h-[500px] p-3 text-white text-xs font-semibold ">
                      {new Date(post.pubDate).toUTCString().split(" ").slice(0, 4).join(" ")}
                    </span>
                  </Squircle>
                </div>
              </Squircle>
              <div className="p-5 md:p-8">
                ...
                <Squircle className="bg-primary w-[25%] md:w-[20%]" radius={90}>
                  <span className="w-10 text-white ms-2 text-xs font-semibold">
                    {post.categories[0]} {/* Categoria del post */}
                  </span>
                </Squircle>

                <h3 className="title-animation text-xl 2xl:text-3xl font-bold text-gray-900 mt-5">
                  {post.title} {/* Titolo del post */}
                </h3>
                <p className="text-gray-700 mt-5">{post.content}</p>
                <a
                  href={post.link} // Link al post
                  target="_blank"
                  rel="noreferrer"
                  className=" text-primary hover:underline mt-5"
                >
                  Read more
                </a>
              </div>
